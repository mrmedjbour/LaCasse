...
@section('content')
    <section id="sectionContent">
        <div class="container">
            <div class="row">
                <div class="col">
                    <h1 class="searchResultTitle">{{__('Search results')}} : {{ Str::title($request->partTitle.' '.$request->makeTitle.' '.$request->modeleTitle) }} {{ $request->year? "- $request->year":'' }}</h1>
                </div>
            </div>
            <div class="row">
                <div class="col-12 col-md-8 order-2 order-md-1" id="content">
                    @if($result->count() == 0)
                        <div class="text-center py-4">
                            ...
                            <h3 class="text-black-50">{{__('No Results')}}</h3>
                            <span class="font-weight-bold">{{__('Sorry, We couldn\'t find that part try again later')}}</span>
                        </div>
                    @else
